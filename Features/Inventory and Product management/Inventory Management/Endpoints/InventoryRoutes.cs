...
namespace ArpellaStores.Features.Inventory_and_Product_management.Inventory_Management.Endpoints;
...
public class InventoryRoutes : IRouteRegistrar
{
    ...
    public void MapInventoryRoutes(WebApplication webApplication)
    {
        ...
        app.MapGet("/inventory/{id}", (int id) => this._inventoryService.GetInventory(id)).Produces(200).Produces(404).Produces<Inventory>();
        ...
        app.MapPut("/inventory/{id}", (Inventory inventory, int id) => this._inventoryService.UpdateInventory(inventory, id)).Produces(200).Produces(404).Produces<Inventory>();
        app.MapDelete("/inventory/{id}", (int id) => this._inventoryService.RemoveInventory(id)).Produces(200).Produces(404).Produces<Inventory>();
