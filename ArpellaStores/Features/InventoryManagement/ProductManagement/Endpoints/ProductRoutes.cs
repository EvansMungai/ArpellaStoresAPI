...
public class ProductRoutes : IRouteRegistrar
{
    ...
    public void MapProductRoutes(WebApplication webApplication)
    {
        ...
        app.MapGet("/paged-products", (ProductHandler handler, int pageSize, int pageNumber) => handler.GetPagedProducts(pageNumber, pageSize)).Produces(200).Produces(404).Produces<List<Product>>();
        ...
        app.MapPut("/product/{id}", (ProductHandler handler, Product product, int id) => handler.UpdateProductDetails(product, id)).Produces(200).Produces(404).Produces<Product>().AddEndpointFilter<ValidationEndpointFilter<Product>>();
